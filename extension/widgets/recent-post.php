...
class beecolor_recent_post_widget extends WP_Widget {
    ...
    public function __construct() {
        ...
        $widget_ops = array(
            'classname'     =>  'beecolor_recent_post_widget',
            'description'   =>  esc_html__( 'A widget show post', 'beecolor' ),
        );

        parent::__construct( 'beecolor_recent_post_widget', 'Basic Theme: Recent Post', $widget_ops );
    ...
    function form( $instance ) {
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'select_cat' ) ); ?>">
                <?php esc_attr_e( 'Select Categories:', 'beecolor' ); ?>
            </label>
            ...
            <select id="<?php echo esc_attr( $this->get_field_id( 'select_cat' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'select_cat' ) ) . '[]'; ?>" class="widefat" size="10" multiple>
                ...
                <option value="0" <?php echo ( in_array( 0, $select_cat ) ? 'selected="selected"' : '' ); ?>>
                    <?php esc_html_e( 'All Category', 'beecolor' ); ?>
                </option>
        ...
        <p>
            ...
            <select id="<?php echo esc_attr( $this->get_field_id( 'order' ) ); ?>" name="<?php echo $this->get_field_name('order') ?>" class="widefat">
                <option value="ASC" <?php echo ( $order == 'ASC' ) ? 'selected' : ''; ?>>
                    <?php esc_html_e( 'ASC', 'beecolor' ); ?>
                </option>

                <option value="DESC" <?php echo ( $order == 'DESC' ) ? 'selected' : ''; ?>>
                    <?php esc_html_e( 'DESC', 'beecolor' ); ?>
                </option>
            </select>
        </p>
        ...
        <p>
            ...
            <select id="<?php echo esc_attr( $this->get_field_id( 'order_by' ) ); ?>" name="<?php echo $this->get_field_name('order_by') ?>" class="widefat">
                <option value="ID" <?php echo ( $order_by == 'ID' ) ? 'selected' : ''; ?>>
                    <?php esc_html_e( 'ID', 'beecolor' ); ?>
                </option>

                <option value="date" <?php echo ( $order_by == 'date' ) ? 'selected' : ''; ?>>
                    <?php esc_html_e( 'Date', 'beecolor' ); ?>
                </option>
        ...
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'number' ) ); ?>">
                <?php esc_html_e( 'Number of posts to show:', 'beecolor' ); ?>
            </label>
...
// Register widget
function beecolor_recent_post_widget_register() {
    register_widget( 'beecolor_recent_post_widget' );
}

add_action( 'widgets_init', 'beecolor_recent_post_widget_register' );